...
ps_nc_FL <- subset_samples(ps_nc, Lifestyle == "Free-Living")
ps_nc_PA <- subset_samples(ps_nc, Lifestyle == "Particle attached")

# ACE, Shannon, Simpson, Chao1
## FL
p <- plot_richness(ps_nc_FL, x = "Month", measures=c("ACE", "Shannon", "Simpson", "Chao1"))

boxplots_FL <- p + geom_boxplot(aes(colour = Month)) + 
...
## PA
p <- plot_richness(ps_nc_PA, x = "Month", measures=c("ACE", "Shannon", "Simpson", "Chao1"))

boxplots_PA <- p + geom_boxplot(aes(colour = Month)) + 
